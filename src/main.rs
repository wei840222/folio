...
async fn init_temporal(
    ...
    let client_options = sdk_client_options(url.clone())
        .identity(hostname::get()?.to_string_lossy().to_string())
        .build()?;
    ...
    // Start Worker inside the thread
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            let core_worker = init_worker(&runtime, worker_config, client_for_worker).unwrap();
            let mut worker = Worker::new_from_core(Arc::new(core_worker), task_queue);

            worker.register_wf("FileExpireWorkflow", workflow::file_expire_workflow);
            worker.register_activity("DeleteFileActivity", workflow::delete_file_activity);
